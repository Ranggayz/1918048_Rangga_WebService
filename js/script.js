const ApiKey = "40c322e18ef84843b82d26d7e9dc05fe";
...
function showTeamInfo(id) {
    ...
    // contents.innerHTML = "HALLO SAYANG" + url;

    fetch(url, fetchHeader)
        .then(response => response.json())
        .then(balBalan => {
            console.log(balBalan);
            let sepakbola = `
                    <img src="${balBalan.crestUrl}" alt="${balBalan.name}" width="50px"
                <div class="card-content">
                    ...
                    <p> Berdiri: ${balBalan.founded} <br>
                        Stadion: ${balBalan.venue} <br>
                        Warna Kebanggan: ${balBalan.clubColors} <br> <br>
                        Tempat Dibentuk: ${balBalan.address} <br>
                        Nama Samaran : ${balBalan.shortName}  <br>
                        Singkatan Team : ${balBalan.tla}  <br>
                        Area : ${balBalan.area.name} <br> <br>

                        contact : ${balBalan.phone} /
                        website : ${balBalan.website} /
                        e-mail : ${balBalan.website} 
                    </p>
                </div>
            `;
            document.getElementById('modal-content').innerHTML = sepakbola;
        });
